...
for i= 1:sz(1)
    ...
    while((j<h(i,1)-1)&&(5*(j+2)<sz(2)))
        ...
        shifts2(end+1)=h(i,5*j-2)-h(i,5*(j+1)-2); %h(i,3);
        shifts3(end+1)=sqrt((h(i,5*j)-h(i,5*(j+1)))*(h(i,5*j)-h(i,5*(j+1)))+(h(i,5*j-1)-h(i,5*(j+1)-1))*(h(i,5*j-1)-h(i,5*(j+1)-1)));
        jk=jk+1;
        finish=h(i,5*j);
        finish1=h(i,5*j-1);
        end;
...
for i=1:length(Xtraj)
    if (Xtraj(i)~=0)
    Xtraj(i)=ceil(Xtraj(i)/(1100/128));
    Ytraj(i)=ceil(Ytraj(i)/(1100/128));
    if Xtraj(i)==0
        Xtraj(i)=1;
    end;
    if Ytraj(i)==0
